...
$select="select * from menu where foodID=$id";
$sql=mysql_query($select,$conn) or die(mysql_error());
if($array=mysql_fetch_array($sql)){
    if($_POST['state']=="有")
        mysql_query("update menu set state=1 where foodID=$id",$conn ) or die(mysql_error());
    else
        mysql_query("update menu set state=0 where foodID=$id",$conn ) or die(mysql_error());
