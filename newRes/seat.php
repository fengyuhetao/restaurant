...
if(isset($_GET['page'])){
	  ...
	  $page_count=5;
	  $select=mysql_query("select * from seat",$conn);
	  $row=mysql_num_rows($select);
	  $page_page=ceil($row/$page_count);
	  $offect=($page-1)*$page_count;   //获取上一页的最后一条记录，从而计算下一页的起始记录
	  $selects=mysql_query("select * from seat where seatID order by seatID asc limit $offect,$page_count",$conn);

require "boot.php";
require_once('conn/conn.php');
include("seatRight.php");
?>
